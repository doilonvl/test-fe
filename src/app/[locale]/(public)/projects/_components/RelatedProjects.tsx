...
export default function RelatedProjects({ title, items }: Props) {
  ...
  const cards = useMemo(
    () =>
      items.map((p) => ({
        project: p,
        title: p.project || "",
        subtitle: p.scope || p.client || "",
        image: p.images?.[0]?.url || "",
        href: {
          pathname: "/projects/[slug]",
          params: { slug: p.slug ?? p._id },
        },
      })),
    [items]
  );
        ...
        <>
          ...
          <div className="relative">
            <ul
              ...
              aria-label={title}
            >
              {cards.map(({ project, title, subtitle, image, href }) => (
                <li
                  key={project._id}
                  className="min-w-[260px] max-w-[300px] snap-start"
                >
                  <Link
                    // @ts-expect-error - href type mismatch
                    href={href}
